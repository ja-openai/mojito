...
fn run_with_suppressed_parser_panics(root: &Path, baseline_path: &Path) {
    ...
    let checks = [
        ...
        FileCheck {
            path: "tests/functions/percent.json",
            mode: CheckMode::Runtime,
        },
        FileCheck {
            path: "tests/functions/currency.json",
            mode: CheckMode::Runtime,
        },
        FileCheck {
        ...
        FileCheck {
            path: "tests/functions/offset.json",
            mode: CheckMode::Runtime,
        },
        FileCheck {
            path: "tests/functions/integer.json",
            mode: CheckMode::Runtime,
        },
        FileCheck {
...
fn check_data_model_error_test(defaults: &TestProperties, test: &OfficialTest) -> bool {
    ...
    if expected_codes.is_empty() {
        return test.exp.as_ref().is_some_and(|expected| {
            ...
            .is_ok_and(|actual| actual.is_ok() && &actual.value == expected)
        });
    }
...
fn check_runtime_test(defaults: &TestProperties, test: &OfficialTest) -> bool {
    ...
    let expected_codes = expected_local_codes(defaults, test);
    let actual = match format_with_registry(
        model,
        &args,
        &locale(defaults, test),
...
fn official_test_function(call: FunctionCall<'_>) -> Result<String, mojito_mf2::Diagnostic> {
    let state = official_test_state_from_call(&call)?;
    if state.fails_format {
        return Err(mojito_mf2::Diagnostic::new(
            "bad-option",
...
fn official_test_select_resolver(call: FunctionCall<'_>) -> Result<String, mojito_mf2::Diagnostic> {
    Ok(official_test_state_from_call(&call)?.format_value())
}

fn official_test_format_resolver(call: FunctionCall<'_>) -> Result<String, mojito_mf2::Diagnostic> {
    Ok(official_test_state_from_call(&call)?.format_value())
}

fn official_test_selector(call: FunctionMatch<'_>) -> Result<Option<i32>, mojito_mf2::Diagnostic> {
...
) -> Result<Option<i32>, mojito_mf2::Diagnostic> {
    Err(mojito_mf2::Diagnostic::new(
        "bad-selector",
...
fn official_test_state_from_call(
    call: &FunctionCall<'_>,
) -> Result<OfficialTestState, mojito_mf2::Diagnostic> {
...
fn official_test_state(
    ...
    option_value: impl Fn(&str) -> Result<Option<String>, mojito_mf2::Diagnostic>,
) -> Result<OfficialTestState, mojito_mf2::Diagnostic> {
...
fn arguments_for(test: &OfficialTest, model: &MessageModel) -> Arguments {
    let mut arguments = Arguments::new();
    for declaration in declarations(model) {
        if let Declaration::Input { name, .. } = declaration {
            arguments.insert(name.clone(), "1");
        }
    }
    for param in &test.params {
        arguments.insert(param.name.clone(), argument_from_json(&param.value));
    }
    arguments
}

fn runtime_arguments_for(test: &OfficialTest) -> Arguments {
    let mut arguments = Arguments::new();
    for param in &test.params {
        arguments.insert(param.name.clone(), argument_from_json(&param.value));
    }
