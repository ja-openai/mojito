...
existing meanings. Direct callers can use `format(...)` or `formatToParts(...)`
with defaulted options. The shared `number-core` fixture checks static outputs,
...
Direct callers can use `formatDate(...)`, `formatTime(...)`,
`formatDateTime(...)`, and the corresponding `format*ToParts(...)` wrappers with
defaulted options. The shared `date-time-core` fixture checks static outputs, semantic skeleton
