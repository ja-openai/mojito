...
private let functions = sampleCatalogRegistry()
...
let recoveryModel = parseToModel("Hello {$name}").model!
let recovered = try formatMessage(
    recoveryModel,
    onMissingArgument: { context in "[missing \(context.variableName ?? "value")]" }
)
guard recovered.value == "Hello [missing name]", recovered.errors.count == 1 else {
    throw DemoError.formatErrors(recovered.errors.map(\.code))
}
print("recovery[en] -> \"\(recovered.value)\"")
...
private struct Catalog: Decodable {
    ...
    func translate(
        ...
        if result.hasErrors {
            throw DemoError.formatErrors(result.errors.map(\.code))
        }
        return result.value
    }
    ...
    private func model(_ messageID: String, locale: String) throws -> MF2Message {
        ...
        guard let fallback = lookupLocale(in: localized, locale: locale) else {
...
private func lookupLocale<T>(in values: [String: T], locale: String, fallback: String = "en") -> T? {
    var canonicalValues: [String: T] = [:]
    for (key, value) in values {
        canonicalValues[canonicalLocaleKey(key)] = value
    }
    ...
    return canonicalValues[canonicalLocaleKey(fallback)]
}
...
private func sampleCatalogRegistry() -> MF2FunctionRegistry {
