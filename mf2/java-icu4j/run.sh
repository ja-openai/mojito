...
build() {
  mvn_cmd -q -f ../java/pom.xml install -DskipTests
...
case "$command" in
  demo)
    java -cp "$tool_classpath" com.box.l10n.mojito.mf2.icu4j.Icu4jRegistryDemo
    ;;
  check)
    java -cp "$tool_classpath" com.box.l10n.mojito.mf2.icu4j.Icu4jRegistryDemo --quiet
    ;;
  *)
