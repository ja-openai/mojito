...
func checkUnicodeDataModelTest(defaults map[string]any, test map[string]any) bool {
	...
	if len(expectedCodes) == 0 {
		...
		actual := FormatMessage(result.Model, unicodeArgumentsFor(test, result.Model), Options{
			Locale:        unicodeLocale(defaults, test),
			BidiIsolation: unicodeBidiIsolation(defaults, test),
		})
		return actual.Ok() && actual.Value == stringValue(test["exp"])
	}
...
func checkUnicodeRuntimeTest(defaults map[string]any, test map[string]any) bool {
	...
	expectedCodes := expectedUnicodeLocalCodes(defaults, test)
	actual := FormatMessage(result.Model, unicodeRuntimeArgumentsFor(test), Options{
		Locale:        unicodeLocale(defaults, test),
		BidiIsolation: unicodeBidiIsolation(defaults, test),
...
func officialGoFunctionRegistry() FunctionRegistry {
	return DefaultFunctionRegistry().
		WithFunction("currency", officialGoCurrencyFunction).
		WithFunction("datetime", officialGoDateTimeFunction).
		WithFunction("date", officialGoDateFunction).
		WithFunction("time", officialGoTimeFunction).
...
func officialGoCurrencyFunction(call FunctionCall) (string, error) {
	value, err := parseCallDecimal(call, "Currency function requires a numeric operand.")
	if err != nil {
		return "", err
	}
	currency := officialGoCurrencyCode(call)
	if currency == "" {
		return "", badOperand("Currency function requires a currency option.")
	}
	digits, err := officialGoCurrencyFractionDigits(call)
	if err != nil {
		return "", err
	}
	number, err := formatUnlocalizedDecimal(value, false, 0)
	if err != nil {
		return "", err
	}
	if digits != nil {
...
func officialGoCurrencyCode(call FunctionCall) string {
	if value, _ := call.OptionValue("currency", ""); value != "" {
		return value
	}
	return officialGoInheritedCurrencyCode(call.InheritedSource)
}

func officialGoInheritedCurrencyCode(source *FunctionSource) string {
	if source == nil {
		return ""
	}
	...
	return officialGoInheritedCurrencyCode(source.Inherited)
}

func officialGoCurrencyFractionDigits(call FunctionCall) (*int, error) {
...
func officialGoDateTimeFunction(call FunctionCall) (string, error) {
	if officialGoISODate(call.Value) || officialGoISODateTime(call.Value) {
		return call.Value, nil
	}
	return "", badOperand("Datetime function requires a date or datetime operand.")
}

func officialGoDateFunction(call FunctionCall) (string, error) {
	if officialGoISODate(call.Value) || officialGoISODateTime(call.Value) {
		return call.Value, nil
	}
...
func officialGoTimeFunction(call FunctionCall) (string, error) {
	if officialGoISODateTime(call.Value) {
		return call.Value, nil
	}
	return "", badOperand("Datetime and time functions require a datetime operand.")
}

func officialGoISODateTime(value string) bool {
	separator := strings.Index(value, "T")
	return separator >= 0 && officialGoISODate(value[:separator]) && officialGoISOTime(value[separator+1:])
}

func officialGoISODate(value string) bool {
...
func officialGoISOTime(value string) bool {
