...
JDK-backed formatting. The shared `number-core` fixture checks static outputs,
...
for the current probe locale set. Use `Mf2DateTimeCore.registry()` when a caller wants the generated-data date/time
handlers explicitly; `Mf2FunctionRegistry.defaults()` continues to mean
JDK-backed formatting. The shared `date-time-core` fixture checks static
outputs, semantic skeleton outputs, error codes, registry integration, and
`DateTimeFormatter` reference behavior.
