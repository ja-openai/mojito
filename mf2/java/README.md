...
JDK-backed formatting. Direct Java callers can use `format(value)` and
`formatToParts(value)` for default options, or the corresponding `(value,
options)` overloads with the builder returned by `Mf2NumberCore.options()`. The
shared `number-core` fixture checks static
