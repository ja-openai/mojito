...
final class FormattedPartJson {
    ...
    static List<Map<String, Object>> toMaps(List<Mf2FormattedPart> parts) {
        return parts.stream().map(FormattedPartJson::toMap).toList();
    }

    static Map<String, Object> toMap(Mf2FormattedPart part) {
        Map<String, Object> map = new LinkedHashMap<>();
        switch (part) {
            case Mf2FormattedPart.Text text -> {
                map.put("type", "text");
            ...
            case Mf2FormattedPart.Markup markup -> {
                map.put("type", "markup");
