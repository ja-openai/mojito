...
OUTPUT_TARGETS = ("json", "python", "rust", "swift", "java", "javascript", "go", "php")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--targets",
        default="all",
        help="Comma-separated output targets: json,python,rust,swift,java,javascript,go,php, or 'all'.",
    )
    parser.add_argument(
        "--java-package",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Write JavaScript generated_plural_rules.js directly under --out instead of --out/javascript.",
    )
    parser.add_argument(
        "--go-package",
        default="mf2",
    ...
    parser.add_argument(
        "--go-source-root",
        action="store_true",
        help="Write Go plural_rules.go directly under --out instead of --out/go.",
    )
    parser.add_argument(
        "--php-source-root",
        action="store_true",
        help="Write PHP GeneratedPluralRules.php directly under --out instead of --out/php.",
    )
    parser.add_argument(
        "--clean",
        action="store_true",
...
def write_outputs(
    ...
    javascript_path = (
        out / "generated_plural_rules.js"
        if javascript_source_root
        else out / "javascript" / "plural_rules.js"
    )
    go_path = out / "plural_rules.go" if go_source_root else out / "go" / "plural_rules.go"
    php_path = out / "GeneratedPluralRules.php" if php_source_root else out / "php" / "GeneratedPluralRules.php"
    paths = {
