...
class InflectionReleaseGateTest(unittest.TestCase):
    ...
    def test_inflection_release_wrapper_rejects_absolute_invalid_bundle_source_paths(
        self,
    ) -> None:
        wrapper = self.load_release_fixture_wrapper()
        with tempfile.TemporaryDirectory(prefix="mojito-mf2-inflection-wrapper-test-") as tmp:
            root_dir = Path(tmp)
            cases = (
                ("absolute", str(root_dir / "outside.json"), "must be relative"),
                ("invalid", "bad\u0000path.json", "is invalid"),
            )
            for label, artifact_path, expected_message in cases:
                with self.subTest(label=label):
                    ...
                    message = str(error.exception)
                    self.assertIn(
                        f"bundle ARTIFACTS artifact path for ar-approved-json {expected_message}",
                        message,
                    )
                    self.assertNotIn(str(root_dir), message)
                    self.assertNotIn(artifact_path, message)
    ...
    def test_release_validator_rejects_absolute_manifest_paths_without_leaking_root(
        self,
    ) -> None:
        forbidden_claim_phrases = (
            "all languages",
        ...
        with tempfile.TemporaryDirectory(prefix="mojito-mf2-inflection-release-test-") as tmp:
            ...
            for index, absolute_path in enumerate(absolute_paths):
                with self.subTest(absolute_path=absolute_path):
                    ...
                    self.assertEqual("Release artifact path must be relative", row["message"])
                    self.assertNotIn(str(root_dir), row["message"])
                    self.assertNotIn(absolute_path, row["message"])
    ...
    def test_release_validator_reports_mixed_failure_codes_without_short_circuiting(
        self,
    ) -> None:
        valid_fixture = (
            REPO_ROOT
        ...
        report = json.loads(result.stdout)
        self.assertEqual({"artifacts", "passed", "failed"}, set(report["summary"]))
        self.assertEqual(4, report["summary"]["artifacts"])
        self.assertEqual(1, report["summary"]["passed"])
        self.assertEqual(3, report["summary"]["failed"])
        self.assertEqual(
            ["escape-json", "invalid-json", "missing-json", "valid-json"],
            [row["artifactId"] for row in report["artifacts"]],
        )
        expected_rows = {
            "escape-json": ("failed", "invalid-release-artifact-path"),
            "invalid-json": ("failed", "invalid-compiled-term-pack-json"),
            "missing-json": ("failed", "unreadable-release-artifact"),
            "valid-json": ("passed", None),
        }
        for row in report["artifacts"]:
            expected_status, expected_code = expected_rows[row["artifactId"]]
            self.assertEqual(expected_status, row["status"])
            if expected_code is None:
                self.assertEqual({"artifactId", "kind", "status"}, set(row))
            else:
                self.assertEqual({"artifactId", "kind", "status", "code", "message"}, set(row))
                self.assertEqual(expected_code, row["code"])
                self.assertNotIn(str(base_dir), row["message"])
    ...
    def test_release_validator_writes_out_report_before_failing_without_allow_failures(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory(prefix="mojito-mf2-inflection-release-test-") as tmp:
            base_dir = Path(tmp)
        ...
        self.assertNotIn("Traceback", row["message"])
        self.assertNotIn(str(base_dir), result.stderr)
        self.assertNotIn(str(base_dir), json.dumps(report, ensure_ascii=False, sort_keys=True))
    ...
    def test_release_validator_rejects_unwritable_report_without_traceback(
        ...
        with tempfile.TemporaryDirectory(prefix="mojito-mf2-inflection-release-test-") as tmp:
            base_dir = Path(tmp)
            artifact_path = base_dir / "valid.json"
            artifact_path.write_text(
                valid_fixture.read_text(encoding="utf-8"),
                encoding="utf-8",
            )
            ...
            report_is_directory = report_path.is_dir()

        self.assertEqual("", result.stdout)
        self.assertEqual(1, result.returncode, result.stderr)
        self.assertTrue(report_is_directory)
        self.assertEqual(
            "Release validation failed: Release validation report is unwritable: "
    ...
    def test_release_validator_rejects_duplicate_manifest_ids_without_traceback(
        self,
    ) -> None:
        forbidden_claim_phrases = (
            "complete locale",
        ...
        self.assertEqual(1, result.returncode, result.stderr)
        self.assertEqual(
            "Release validation failed: Duplicate release manifest artifact ID: "
            "duplicate-json\n",
            result.stderr,
        )
        self.assertNotIn("Traceback", result.stderr)
    ...
    def test_release_validator_rejects_non_text_manifest_artifact_fields_without_report(
        self,
    ) -> None:
        forbidden_claim_phrases = (
            "complete locale",
        ...
        cases = (
            (
                "artifactId",
                {"artifactId": 7, "kind": "compiled-term-pack-json", "path": "artifact.json"},
                "release-validation-manifest.json.artifacts[0].artifactId",
            ),
            (
                "kind",
                {"artifactId": "non-text-kind", "kind": 7, "path": "artifact.json"},
                "release-validation-manifest.json.artifacts[0].kind",
            ),
            (
                "path",
                {"artifactId": "non-text-path", "kind": "compiled-term-pack-json", "path": 7},
                "release-validation-manifest.json.artifacts[0].path",
            ),
        )
        for field, artifact, expected_label in cases:
    ...
    def test_release_validator_rejects_unexpected_manifest_keys_without_report(
        self,
    ) -> None:
        forbidden_claim_phrases = (
            "complete locale",
        ...
        cases = (
            (
                "top-level",
                {**base_manifest, "generatedAt": "2026-07-05T00:00:00Z"},
                    ...
                    "artifacts": [
                        {
                            **base_manifest["artifacts"][0],
                            "checksum": "not-part-of-manifest-contract",
                        }
                    ],
                },
                "release-validation-manifest.json.artifacts[0]",
                "checksum",
            ),
        )
        for label, manifest, expected_label, expected_key in cases:
            with self.subTest(label=label):
                with tempfile.TemporaryDirectory(
                    prefix="mojito-mf2-inflection-release-test-"
                ) as tmp:
                ...
                self.assertFalse(report_exists)
                self.assertEqual(
                    "Release validation failed: Unexpected keys in "
                    f"{expected_label}: ['{expected_key}']\n",
                    result.stderr,
                )
                self.assertNotIn("Traceback", result.stderr)
                normalized_stderr = result.stderr.lower()
    ...
    def test_release_validator_rejects_manifest_shape_errors_without_report(
        self,
    ) -> None:
        forbidden_claim_phrases = (
            "complete locale",
        ...
        cases = (
            ...
            (
                ...
                "Expected object: release-validation-manifest.json.artifacts[0]",
            ),
        )
        for label, manifest, expected_error in cases:
            with self.subTest(label=label):
    ...
    def test_tracker_current_checkpoint_mentions_latest_release_slice_guards(self) -> None:
        ...
        for snippet in (
            ...
            "Latest release-wrapper bundle-source path hygiene guard",
        ...
        for snippet in (
            ...
            "does not echo the malformed path or local temporary root",
            "The Python package harness now passes 129 tests",
            "Two thousand one hundred fifty-ninth release-wrapper bundle-source path hygiene guard",
    ...
    def test_inflection_release_wrapper_rejects_absolute_manifest_path_without_leaking_root(
        self,
    ) -> None:
        wrapper = self.load_release_fixture_wrapper()
        with tempfile.TemporaryDirectory(prefix="mojito-mf2-inflection-wrapper-test-") as tmp:
    ...
    def test_inflection_release_wrapper_rejects_missing_manifest_report_artifact_fields(self) -> None:
        wrapper = self.load_release_fixture_wrapper()
        cases = (
            (
