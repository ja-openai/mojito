  ...
  for (const fixturePath of await jsonFiles(sourceDir)) {
    ...
    for (const formatCase of fixture.formatCases ?? []) {
      ...
      if (actual !== formatCase.expected) {
        throw new ConformanceFailure(`${basename(fixturePath)}: expected ${JSON.stringify(formatCase.expected)}, got ${JSON.stringify(actual)}`);
      }
      checkedCases += 1;
    }
    ...
    for (const partsCase of fixture.partsCases ?? []) {
      ...
      assertDeepEqual(partsCase.expected, actual, `${basename(fixturePath)}: parts did not match`);
      checkedPartsCases += 1;
    }

    for (const fallbackCase of fixture.fallbackCases ?? []) {
      const actual = formatMessageWithFallback(parseResult.model, fallbackCase.arguments ?? {}, {
    ...
    for (const partsCase of fixture.fallbackPartsCases ?? []) {
      const actual = formatMessageToPartsWithFallback(parseResult.model, partsCase.arguments ?? {}, {
        locale: partsCase.locale ?? "en",
      });
...
async function checkFormatErrorFixtures(fixtureRoot) {
  ...
  for (const fixturePath of await jsonFiles(fixtureDir)) {
    ...
    try {
      ...
      throw new ConformanceFailure(`${basename(fixturePath)}: expected format error, got ${JSON.stringify(actual)}`);
    } catch (error) {
