...
future `Intl` adapter can provide locale-pretty platform formatting without
changing the core registry boundary.

The package exposes a single root import. Locale lookup, CLDR plural-rule
helpers, `partsToString`, `selectPluralCategory`, and `valueToString` are
formatter internals rather than public package subpaths, matching the Java
