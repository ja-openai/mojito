import { formatMessage, formatMessageToParts, parseToModel } from "../src/index.js";
...
for (const [id, source] of Object.entries(catalog)) {
  ...
  console.log(`${id} -> ${JSON.stringify(formatMessage(parsed.model, args, { locale: "en" }).value)}`);
  if (id === "rich.link") {
    console.log(`${id} parts -> ${JSON.stringify(formatMessageToParts(parsed.model, args, { locale: "en" }).parts)}`);
  }
}
