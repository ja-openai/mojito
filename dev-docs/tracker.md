...
- QUEUE-02 — Decide how to run the existing MySQL/PostgreSQL Testcontainers queue contract in CI or nightly. Local unit runs compile it but skip container startup unless `-Dmojito.asyncJobQueue.testcontainers=true` is set.
- QUEUE-03 — Extend operator visibility beyond status gauges for failed async jobs: list/search recent failures, inspect `last_error`/payload metadata, expose safe replay/dead-letter handling, and wire terminal-row retention policy on top of the store cleanup primitive.
