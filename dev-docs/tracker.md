...
- GLOSSARY-08 — Extend glossary MCP beyond list/search/bulk-upsert with server-side candidate review, dedicated reference-link operations, duplicate/merge recommendations, and translation update workflows that reuse normal TM review status. See `dev-docs/design/012-glossary-mcp.md`.
- AI-TRANSLATE-01 — Expose normalized AI Translate lineage in an operator-facing surface: text-unit history should show provider ids/status and link to the shared request/response blobs without embedding the payload in translation comments.
