...
- DOC-01 — Refresh published install/contributor docs for the current Spring Boot 3, JDK 21, MySQL 8, Maven-managed Node/npm, `webapp/new-frontend`, and root-path SPA workflow. Replace old `npm run start-dev` guidance with current Maven/Spring Boot and Vite commands.
...
- DOC-04 — Tie integration docs to dependency cleanup: remove or clearly mark Box, Phrase, and Smartling docs when their code paths are removed; do not drop Chrome ICT extension docs/assets until the extension is ported, split, or intentionally retired.
...
- TM-01 — Finish the `TextUnitSearcher` Hibernate Criteria replacement. The candidate implementation and shadow comparator live under `service.tm.search.replacement`; broad HSQL `webapp` and `cli` parity pass with `l10n.text-unit-searcher.shadow.enabled=true` and `fail-on-mismatch=true`. Next step is MySQL parity, then making Criteria primary and removing `nativeCriteria-core`. HQL/native-SQL alternatives are documented but no longer kept in code. See `dev-docs/design/014-text-unit-searcher-replacement.md`.
