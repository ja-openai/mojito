...
cannot be blindly combined with M's different V100. The older queue branch moved
to V109; the integration of queue `086f26b16c` with master `ed4bc31eda` now uses
V113 for both dialects, with unchanged SQL and a version-collision test. Master's
V109 through V112 are preserved. This rename is for the unapplied queue branch;
an environment that applied an older queue migration needs an explicit
history/schema reconciliation, not a blind rename or checksum repair.
...
workers or blindly shift rows. Keep queue SQL unchanged until applied-history and
