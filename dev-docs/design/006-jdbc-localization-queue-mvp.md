...
8. Runtime invokes the handler post-terminal callback to publish that output to PollableTask
   and call `finishTask(...)`. An expired worker cannot overwrite the winning canonical output.
...
- Generic failed-row replay is not yet a supported assetlocalize recovery workflow: reusing an
  already-failed PollableTask retains its error state. Keep replay out of the public admin API until
  fresh-task or explicitly reset-task semantics are defined.
...
- JDBC lease transitions lock the matching owner row before sampling database time, so a wait on
  ...
  `CURRENT_TIMESTAMP(6)` in a fresh statement after the lock. This avoids transaction-start or
