...
- Database constraints reject non-positive ids, unsupported statuses, negative attempt counts, and
  invalid queue names, plus inconsistent lease owner fields where a row is `running` without
  `(lease_until, worker_id, lease_token)` or a non-running row still has lease ownership attached.
