...
cardinal categories from the version-one, audited
`file-formats/conformance/cldr-cardinal-categories.v1.json` manifest. It comes
directly from the
[Unicode CLDR 46 cardinal plural rules](https://github.com/unicode-org/cldr-json/blob/46.0.0/cldr-json/cldr-core/supplemental/plurals.json)
and identifies Unicode 16, 218 supported languages/regions, and the explicitly
non-routable `und` root. Java loads a byte-identical classpath copy and checks
the exact selected rule set against ICU4J; Rust independently includes the
shared manifest at compile time. The FormatJS oracle checks all 218 actual
Node ICU category sets and rejects unsupported tags before ICU can silently
fall back to English. Existing actual same-language plural evidence still
wins, and a present target locale without usable plural evidence cannot bypass
its existing ownership boundary. Original missing/null Russian fixtures have
no other Russian entries; both portable implementations nevertheless insert
exact `one`/`few`/`many`/`other`, anchored source-owned `%3$n`, deterministic
`translated` category states, and untouched protected/German/source records in
both UTF-8 and UTF-16. Real `xcstringstool` compilation and Swift Foundation
execute every Russian category. Unknown locales, the undefined root, and
incomplete category translations fail closed. Apple also documents that Xcode
automatically adds the language-specific categories, including Russian's four
forms, in
...
CLDR version upgrades require an explicit evidence-backed release gate.
Unicode says each
[published CLDR release is immutable](https://cldr.unicode.org/index/downloads)
and requires maintainers to diff regenerated rules and charts whenever plural
rules or categories change in
[Adding/Fixing Plurals and Ordinals](https://cldr.unicode.org/development/updating-codes/update-plurals).
The pinned manifest therefore has exact release-tag provenance, a checked
SHA-256, matching Java/Rust source bytes, and independent Java, Rust, and Node
version guards. An original CLDR 46-to-48.2 comparison found five added rule
locales: `cv`, `ie`, `kok`, `kok-Latn`, and `sgs`, with no changed categories
for existing locales. Current Java ICU4J 78.3 recognizes all five, while
current Node ICU 76.1/CLDR 46 recognizes none. `kok-Latn` additionally exposes
that the current version-one manifest admits only languages and regions, not
script-qualified rule records. The reproducible
`file-formats/conformance/cldr_upgrade_audit.py` accepts an original stable
`plurals.json` and optional upstream SHA-256; it reports exact provenance,
locale additions/removals, category deltas, unsupported shapes/categories,
missing `other`, and a stable fail-closed blocker inventory. `--require-safe`
rejects the 48.2 candidate until locale-shape policy, Node/Java/native-runtime
availability, independent Java/Rust source-template fixtures, CLDR source
hashes, and an intentionally reviewed release upgrade are all resolved. An
unaudited dependency bump can never silently admit new locales or categories.
...
Pinned CLDR gives both `one`/`many`/`other`, but actual Node ICU, Java ICU4J,
...
Pinned CLDR lookup canonicalizes deprecated language aliases before selecting
...
device's
`one`/`few`/`many`/`other` from pinned CLDR. Each scalar/category is
...
only from pinned CLDR, never invent source-absent `%4$n`, retain protected
...
version-pinned CLDR manifest described above; unsupported/undefined locale tags
...
target comments plus pinned-CLDR Android/Apple/PO plural completion, including
