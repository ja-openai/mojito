...
| P0 | Remaining customized workflow integration boundaries | Existing CLI datasets verify configured source extraction, client-side extraction/diff/check, localized output, opt-in native localized-asset import, sync/async/parallel transport, stable properties/Apple/YAML TM identities, Apple owner comments/usages, Russian `.stringsdict` category completion, and French/Japanese/Russian/Croatian gettext plural import and output. Independent Java/Rust contracts cover target comments, ICU-backed plural copying, empty Android array items, nested YAML sequences, and intentionally corrected legacy output defects; 9 actual customized import-filter comparisons remain available. Malformed Foundation sources and remaining valid plural-dictionary import/cleanup edge cases require native evidence. `oldEscaping=true` deliberately fails closed because it conflicts with AAPT2. | Expand import parity to every valid plural-bearing format, keep native-correct intentional divergences documented, and preserve the default Okapi path. |
