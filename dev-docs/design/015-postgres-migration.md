...
This pass also adds focused Postgres compatibility coverage for review search and review-project
decided-count recomputation. The search tests catch typed-parameter issues such as applying string
functions to null/blank bind parameters, and the recompute test catches native SQL that parsed on
MySQL but not PostgreSQL. Glossary term-index refresh upserts are now routed through dialect-aware
repository fragments: MySQL keeps `insert ignore`, PostgreSQL uses `on conflict do nothing`, and
HSQL keeps a simple JPA fallback for local tests.
