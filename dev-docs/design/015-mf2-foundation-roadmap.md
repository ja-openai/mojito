...
- Mojito UI: best-in-class MF2 translation, review, diagnostics, preview, and
  migration tooling.
- Runtime libraries: production-grade Python and native Swift first, Java/JVM and
  native Kotlin/JVM as the low-friction Mojito/server path, with Rust as the
  parser/tooling/LSP core unless a production Rust runtime becomes a direct
  priority.
...
- generate CLDR cardinal and ordinal plural rules for every CLDR plural locale,
  with locale allowlists available as a build-time packaging option
- format from the Unicode MF2 Interchange Data Model in Rust, Swift, Python,
  Java, Kotlin, JavaScript, Go, and PHP
- ship Swift/Rust/Java embedded runtimes without parser or JSON CLDR data when
  using generated code
- keep registry layers explicit: portable production behavior, platform-backed
  defaults where implemented, and test/demo-only fixture shims
...
- custom app/catalog function packages beyond the core registry layers
