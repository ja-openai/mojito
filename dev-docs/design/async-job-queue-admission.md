...
changing DDL: only while the queue migration (V113 in the master integration) remains
unapplied may admission DDL be folded into the single queue migration per dialect.
If queue DDL was applied anywhere, use a new forward migration
...
| V113 fresh install versus previously applied queue DDL | One queue migration per dialect on never-deployed branch; forward upgrade preserves applied checksums. |
