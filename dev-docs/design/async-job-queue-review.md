...
**The integrated branch is not production-ready; release validation remains required.**
...
- Fresh integration verification passes 1,364 focused tests with 51 database opt-in
  skips across 109 suites, no failures or reruns. The frontend and executable JAR
  build successfully. On a disposable native MySQL 8.4.11 server, both maintained
...
DDL: the queue migration (V113 in this integration) may only be extended if
unapplied; otherwise require a forward migration.
