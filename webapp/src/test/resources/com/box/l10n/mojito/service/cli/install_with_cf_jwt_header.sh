...
#!/usr/bin/env bash
if [ -z "${L10N_RESTTEMPLATE_HEADER_HEADERS_CF_ACCESS_TOKEN:-}" ]; then
  echo "Environment variable L10N_RESTTEMPLATE_HEADER_HEADERS_CF_ACCESS_TOKEN must be set before running this command."
fi
...
if [ -z "${L10N_RESTTEMPLATE_HEADER_HEADERS_CF_ACCESS_TOKEN:-}" ]; then
  echo "Environment variable L10N_RESTTEMPLATE_HEADER_HEADERS_CF_ACCESS_TOKEN must be set before running this installation script."
  exit 1
fi
...
CURL_HEADERS=(
  -H "CF-Access-Token: $L10N_RESTTEMPLATE_HEADER_HEADERS_CF_ACCESS_TOKEN"
)
