...
public class AsyncJobStoreContractTest {
  ...
  @Test
  public void queueNamesHaveExactIdentity() {
    AsyncJobStoreIdentityContract.assertQueueIsolation(asyncJobStore);
  }

  @Test
  public void maintenanceUsesExactQueueIdentity() {
    AsyncJobStoreIdentityContract.assertMaintenanceIsolation(asyncJobStore);
  }

  @Test
  public void expiredClaimsUseExactQueueIdentity() throws Exception {
    AsyncJobStoreIdentityContract.assertExpiredQueueIsolation(asyncJobStore);
  }

  @Test
  public void leaseOwnersHaveExactIdentity() {
    AsyncJobStoreIdentityContract.assertLeaseOwnerIsolation(asyncJobStore);
  }
  ...
  @Test
  public void rejectsInvalidPayloadsWithoutChangingState() {
    for (String payload :
        List.of(
            "x".repeat(AsyncJobQueueValidation.JOB_DATA_MAX_LENGTH + 1),
            "bad\0input",
            "bad\uD800input",
            "bad\uDC00input")) {
      assertInvalidPayloadRejectedWithoutChangingState(payload);
    }
  }

  private void assertInvalidPayloadRejectedWithoutChangingState(String invalidPayload) {
    assertThrows(
        IllegalArgumentException.class,
        () -> asyncJobStore.enqueueNow("assetlocalize", invalidPayload));
    assertThrows(
        IllegalArgumentException.class,
        () -> asyncJobStore.enqueue("assetlocalize", invalidPayload, Instant.now()));
    ...
    assertThrows(
        ...
        () ->
            asyncJobStore.markDone(
                "assetlocalize", id, "worker-a", claimed.leaseToken(), invalidPayload));
    assertThrows(
        IllegalArgumentException.class,
        () ->
            asyncJobStore.requeue(
    ...
    AsyncJobRecord stillRunning = asyncJobStore.getByIds(List.of(id)).get(0);
    assertThat(stillRunning).isEqualTo(claimed);

    assertThat(
            asyncJobStore.markFailed(
                "assetlocalize", id, "worker-a", claimed.leaseToken(), null, "visible failure"))
        .isTrue();
    AsyncJobRecord failed = asyncJobStore.getByIds(List.of(id)).getFirst();
    assertThrows(
        IllegalArgumentException.class,
        () -> asyncJobStore.requeueFailed("assetlocalize", id, Instant.now(), invalidPayload));
    assertThrows(
        IllegalArgumentException.class,
        () -> asyncJobStore.requeueFailedNow("assetlocalize", id, invalidPayload));

    AsyncJobRecord stillFailed = asyncJobStore.getByIds(List.of(id)).get(0);
    assertThat(stillFailed).isEqualTo(failed);
  }
