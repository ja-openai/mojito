...
public class AsyncJobQueuePropertiesValidatorTest {
  ...
  @Test
  public void producerOnlyQueueRejectsProcessLocalStore() {
    ...
    assertThat(exception).hasMessageContaining("consumerEnabled=false requires store=jdbc");
  }

  @Test
  public void producerOnlyQueueAcceptsBothJdbcDialects() throws Exception {
    AsyncJobQueueProperties properties = new AsyncJobQueueProperties();
    ...
    properties.getQueues().put("assetlocalize", settings);
    AsyncJobQueuePropertiesValidator validator = new AsyncJobQueuePropertiesValidator(properties);
    for (String dialect : java.util.List.of("mysql", "postgresql")) {
      properties.setJdbcDialect(dialect);
