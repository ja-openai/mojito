    ...
    try (PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:16-alpine")) {
      ...
      try (Connection connection =
        ...
        assertThat(
            ...
            .isEqualTo(1);
        assertThat(tableExists(connection, "json_config_localization")).isTrue();
        assertThat(tableExists(connection, "json_config_localization_run")).isTrue();
        assertThat(tableExists(connection, "review_project_assignment_window")).isTrue();
        assertThat(tableExists(connection, "review_project_time_spent_stat")).isTrue();
        assertThat(tableExists(connection, "review_project_text_unit_suggestion")).isTrue();
        assertThat(tableExists(connection, "mblob_cleanup_policy")).isTrue();
        assertThat(tableExists(connection, "bulk_import_run")).isTrue();
        assertThat(tableExists(connection, "user_preferences")).isTrue();
        assertThat(columnType(connection, "review_project", "decided_word_count"))
            .isEqualTo("bigint");
        assertThat(columnType(connection, "branch", "cleanup_date"))
            .isEqualTo("timestamp without time zone");
        assertThat(
                columnType(
                    connection,
                    "ai_translate_automation_config",
                    "excluded_repository_ids_json"))
            .isEqualTo("text");
        assertThat(
                columnType(
                    connection, "ai_translate_text_unit_attempt", "prompt_fingerprint"))
            .isEqualTo("character varying");
        assertThat(
                columnType(
                    connection, "review_project_time_spent_stat", "decision_interval_count"))
            .isEqualTo("bigint");
        assertThat(columnType(connection, "review_project_text_unit_suggestion", "target"))
            .isEqualTo("text");
        assertThat(columnType(connection, "mblob_cleanup_policy", "enabled"))
            .isEqualTo("boolean");
        assertThat(columnType(connection, "bulk_import_run", "run_id"))
            .isEqualTo("character varying");
        assertThat(columnType(connection, "user_preferences", "preferences_json"))
            .isEqualTo("text");
