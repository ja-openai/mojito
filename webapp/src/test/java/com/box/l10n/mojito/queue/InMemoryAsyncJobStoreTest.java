...
public class InMemoryAsyncJobStoreTest {
  ...
  @Test
  public void relativeRetentionUsesTransitionClockAndPreservesNonterminalRows() throws Exception {
    AsyncJobId done = inMemoryAsyncJobStore.enqueueNow("assetlocalize", "done");
    AsyncJobId failed = inMemoryAsyncJobStore.enqueueNow("assetlocalize", "failed");
    for (AsyncJobRecord job :
        inMemoryAsyncJobStore.claimNextJobs("assetlocalize", 2, "worker", Duration.ofMinutes(1))) {
      assertThat(
              job.id().equals(done)
                  ? inMemoryAsyncJobStore.markDone(
                      "assetlocalize", job.id(), "worker", job.leaseToken(), null)
                  : inMemoryAsyncJobStore.markFailed(
                      "assetlocalize", job.id(), "worker", job.leaseToken(), null, "failed"))
          .isTrue();
    }
    AsyncJobId running = inMemoryAsyncJobStore.enqueueNow("assetlocalize", "running");
    inMemoryAsyncJobStore.claimNextJobs("assetlocalize", 1, "worker", Duration.ofMinutes(1));
    AsyncJobId queued = inMemoryAsyncJobStore.enqueueNow("assetlocalize", "queued");
    List<AsyncJobId> ids = List.of(done, failed, running, queued);
    List<AsyncJobRecord> before = inMemoryAsyncJobStore.getByIds(ids);
    for (AsyncJobStatus status : List.of(AsyncJobStatus.DONE, AsyncJobStatus.FAILED)) {
      assertThat(
              inMemoryAsyncJobStore.deleteTerminalJobsOlderThan(
                  "assetlocalize", status, Duration.ofDays(365), 100))
          .isZero();
    }
    assertThat(inMemoryAsyncJobStore.getByIds(ids)).containsExactlyElementsOf(before);
    Instant latestTerminal =
        before.stream()
            .filter(
                job -> job.status() == AsyncJobStatus.DONE || job.status() == AsyncJobStatus.FAILED)
            .map(AsyncJobRecord::updatedDate)
            .max(Instant::compareTo)
            .orElseThrow();
    long deadline = System.nanoTime() + Duration.ofSeconds(1).toNanos();
    while (!Instant.now().isAfter(latestTerminal.plusNanos(1)) && System.nanoTime() < deadline) {
      Thread.sleep(1);
    }
    assertThat(Instant.now()).isAfter(latestTerminal.plusNanos(1));
    for (AsyncJobStatus status : List.of(AsyncJobStatus.DONE, AsyncJobStatus.FAILED)) {
      assertThat(
              inMemoryAsyncJobStore.deleteTerminalJobsOlderThan(
                  "assetlocalize", status, Duration.ofNanos(1), 1))
          .isEqualTo(1);
    }
    assertThat(inMemoryAsyncJobStore.getByIds(ids)).containsExactlyElementsOf(before.subList(2, 4));
  }

  @Test
  public void relativeRetentionRejectsInvalidAgesAndKeepsEmptyBatchNoOp() {
    for (Duration age :
        List.of(
            Duration.ZERO,
            Duration.ofNanos(-1),
      ...
      assertThrows(
          ...
          () ->
              inMemoryAsyncJobStore.deleteTerminalJobsOlderThan(
                  "assetlocalize", AsyncJobStatus.DONE, age, 100));
    }
    assertThrows(
        NullPointerException.class,
        () ->
            inMemoryAsyncJobStore.deleteTerminalJobsOlderThan(
                "assetlocalize", AsyncJobStatus.DONE, null, 100));
    assertThat(inMemoryAsyncJobStore.deleteTerminalJobsOlderThan(null, null, null, 0)).isZero();
  }
  ...
  public void readyStatusCountsOnlyClaimableQueuedJobs() {
    ...
    inMemoryAsyncJobStore.enqueue("assetlocalize", "{\"name\":\"old-ready\"}", oldReadyAt);
    inMemoryAsyncJobStore.enqueue(
    ...
    AsyncJobReadyStatus readyStatus = inMemoryAsyncJobStore.readyStatus("assetlocalize");
  ...
  public void expiredLeaseStatusCountsOnlyRunningJobsWithExpiredLeases() throws Exception {
    inMemoryAsyncJobStore.enqueue("assetlocalize", "{\"name\":\"expired\"}", Instant.now());
    inMemoryAsyncJobStore
        .claimNextJobs("assetlocalize", 1, "worker-a", Duration.ofMillis(50))
        .get(0);

    assertThat(inMemoryAsyncJobStore.expiredLeaseStatus("assetlocalize").count()).isEqualTo(0);

    Thread.sleep(80);

    AsyncJobExpiredLeaseStatus expiredLeaseStatus =
        inMemoryAsyncJobStore.expiredLeaseStatus("assetlocalize");
