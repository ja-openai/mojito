...
public class AsyncJobQueueValidationTest {
  ...
  @Test
  public void errorSummaryEscapesNulAndUnpairedSurrogates() {
    assertThat(AsyncJobQueueValidation.truncateLastError("bad\0input-\uD800-x-\uDC00"))
        .isEqualTo("bad\\u0000input-\\ud800-x-\\udc00");
    assertThat(AsyncJobQueueValidation.truncateLastError("\uD800\uD83D\uDE80\uDC00"))
        .isEqualTo("\\ud800\uD83D\uDE80\\udc00");
  }

  @Test
  public void errorSummaryPreservesValidUnicodeAndWhitespace() {
    String message = "\t\n\r\\u0000-\u00e9-\uD83D\uDE80";
    assertThat(AsyncJobQueueValidation.truncateLastError(message)).isEqualTo(message);
  }

  @Test
  public void errorSummaryNeverSplitsSupplementaryCharactersAtItsLimit() {
    String prefix = "x".repeat(AsyncJobQueueValidation.LAST_ERROR_MAX_LENGTH - 2);
    assertThat(AsyncJobQueueValidation.truncateLastError(prefix + "\uD83D\uDE80tail"))
        .isEqualTo(prefix + "\uD83D\uDE80");
    assertThat(AsyncJobQueueValidation.truncateLastError(prefix + "x\uD83D\uDE80tail"))
        .isEqualTo(prefix + "x");
  }

  @Test
  public void errorSummaryBoundsExpandedEscapesWithoutSplittingThem() {
    String prefix = "x".repeat(AsyncJobQueueValidation.LAST_ERROR_MAX_LENGTH - 6);
    assertThat(AsyncJobQueueValidation.truncateLastError(prefix + "\0tail"))
        .isEqualTo(prefix + "\\u0000");
    assertThat(AsyncJobQueueValidation.truncateLastError(prefix + "x\uD800tail"))
        .isEqualTo(prefix + "x");
    String normalized = AsyncJobQueueValidation.truncateLastError("\0".repeat(4_000));
    assertThat(normalized).isEqualTo("\\u0000".repeat(666));
    assertThat(AsyncJobQueueValidation.truncateLastError(normalized)).isEqualTo(normalized);
  }
  ...
  @Test
  public void payloadWritesRejectNulAndUnpairedSurrogatesWithoutRewritingThem() {
    for (String payload :
        new String[] {
          "bad\0input",
          "bad\uD800input",
      ...
      assertThrows(
          IllegalArgumentException.class, () -> AsyncJobQueueValidation.validateJobData(payload));
      assertThrows(
          IllegalArgumentException.class,
          () -> AsyncJobQueueValidation.validateOptionalJobData(payload));
    }
  }

  @Test
  public void payloadWritesPreserveUnicodeEscapesWhitespaceAndUtf16Limit() {
    for (String payload :
        new String[] {
          "",
          "\t\n\r\\u0000-\\uD800-\u00e9-e\u0301-\uD83D\uDE80",
      ...
      assertThat(AsyncJobQueueValidation.validateJobData(payload)).isSameAs(payload);
    }
    assertThrows(
        IllegalArgumentException.class,
        () -> AsyncJobQueueValidation.validateJobData("x".repeat(999_999) + "\uD83D\uDE80"));
  }
