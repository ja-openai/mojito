...
public class AsyncJobHandlerResultTest {
  ...
  @Test
  public void rejectsNonPortableReplacementTextButAllowsKeepingExistingPayload() {
    for (String payload : new String[] {"bad\0input", "bad\uD800input", "bad\uDC00input"}) {
      assertThrows(IllegalArgumentException.class, () -> AsyncJobHandlerResult.done(payload));
      assertThrows(
          IllegalArgumentException.class, () -> AsyncJobHandlerResult.requeue(null, payload));
    }
    assertThat(AsyncJobHandlerResult.done().jobData()).isNull();
    assertThat(AsyncJobHandlerResult.requeue(null).jobData()).isNull();
  }
}
