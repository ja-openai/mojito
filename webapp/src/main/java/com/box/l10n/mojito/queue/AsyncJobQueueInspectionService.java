    ...
    try {
      ...
      recordReplayDiagnostic(
          () ->
              meterRegistry
                  .counter(
                      "asyncJobQueue.inspection.requeueWakeup.notify.failed",
                      "queueName",
                      queueName)
                  .increment());
      warnReplaySafely(
  ...
  private void incrementRequeueCounter(String queueName, String result) {
    recordReplayDiagnostic(
        () ->
            meterRegistry
                .counter(
                    "asyncJobQueue.inspection.requeue", "queueName", queueName, "result", result)
                .increment());
  }

  private void infoReplaySafely(String message, Object... arguments) {
    recordReplayDiagnostic(() -> logger.info(message, arguments));
  }

  private void warnReplaySafely(String message, Object... arguments) {
    recordReplayDiagnostic(() -> logger.warn(message, arguments));
  }

  private void recordReplayDiagnostic(Runnable recording) {
    try {
      recording.run();
      ...
      try {
        logger.warn("Failed to record async job replay diagnostic", failure);
      } catch (Throwable loggingFailure) {
        Error loggingFatal = AsyncJobQueueFatalErrors.findJvmFatal(loggingFailure);
        if (loggingFatal != null) {
          throw loggingFatal;
        }
        // Do not recurse or turn a diagnostic failure into an apparent replay failure.
      }
    }
  }
