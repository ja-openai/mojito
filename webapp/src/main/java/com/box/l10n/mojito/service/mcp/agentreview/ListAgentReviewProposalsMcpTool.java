...
@Component
public class ListAgentReviewProposalsMcpTool
    extends AgentReviewMcpTool<ListAgentReviewProposalsMcpTool.Input> {
  private static final McpToolDescriptor DESCRIPTOR =
      new McpToolDescriptor(
          "agent_review.list_proposals",
          "List staged review proposals",
          "Read a bounded page of run proposals, including original baseline, proposed text, evidence, readiness, and human disposition. Requires PM/admin with run access. Follow nextAfterId until null; a full final page may be followed by an empty page.",
          true,
          true,
  ...
  public ListAgentReviewProposalsMcpTool(
  ...
  protected Object executeAuthorized(Input input) {
    ...
    List<AgentReviewProposal> proposals =
        service.listProposals(requireId(input.runId(), "runId"), cursor(input.afterId()), limit);
    return new Result(
