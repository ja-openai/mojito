...
public enum AiTranslateType {
  ...
  private static final String TRANSLATION_QUALITY_INSTRUCTIONS =
      ...
      - Preserve placeholders, tags, URLs, and code. For ICU MessageFormat and MessageFormat 2, preserve variable names, selector expressions, and formatting behavior; translate every text branch while retaining its condition. Follow the supplied plural guidance for locale-appropriate branches and naturally expressed fixed quantities. Preserving placeholders does not require printing the selected count in every branch or copying only the source language's plural categories; preserve unrelated placeholders and information.
