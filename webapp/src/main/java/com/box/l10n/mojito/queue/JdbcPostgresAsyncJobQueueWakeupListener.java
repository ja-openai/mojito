...
class JdbcPostgresAsyncJobQueueWakeupListener implements SmartLifecycle {
  ...
  JdbcPostgresAsyncJobQueueWakeupListener(
    ...
    this.dataSource = JdbcPostgresAsyncJobQueueWakeupConnections.independentDataSource(dataSource);
    this.channel =
  ...
  public void stop() {
    ...
    synchronized (lifecycleLock) {
      running = false;
      // Only the listener thread may clean up JDBC state; even pooled close() can block.
    ...
    if (threadToJoin != null && threadToJoin != Thread.currentThread()) {
      try {
        ...
        if (threadToJoin.isAlive()) {
          incrementCounter("asyncJobQueue.wakeup.listener.stopTimeout");
          logger.warn(
        ...
        incrementCounter("asyncJobQueue.wakeup.listener.stopInterrupted");
      }
    }
    synchronized (lifecycleLock) {
    ...
    try {
      while (running) {
        try {
          ...
          Error fatal = AsyncJobQueueFatalErrors.findJvmFatal(exception);
          if (fatal != null) {
            throw fatal;
          }
          if (running) {
  ...
  private void listenUntilDisconnected() throws Exception {
    ...
    try (AutoCloseable ownedConnection = () -> closeConnection(connection);
      ...
      try (AutoCloseable subscription = () -> unlisten(connection)) {
        listen(connection);
        Object pgConnection = unwrapPgConnection(connection, pgConnectionClass);
        incrementListenCounter("connected");
        while (running) {
          Object[] notifications = readNotifications(getNotifications, pgConnection);
  ...
  private Object[] readNotifications(Method getNotifications, Object pgConnection)
      throws Exception {
    try {
      return (Object[]) getNotifications.invoke(pgConnection, (int) listenTimeoutMs);
    } catch (InvocationTargetException exception) {
      // Unwrap before resource cleanup so its failures attach to the original driver error.
  ...
  private void unlisten(Connection connection) throws SQLException {
    if (connection.isClosed()) {
      return;
    }
    try (Statement statement = connection.createStatement()) {
      // LISTEN is session state: returning a pooled connection does not remove it.
      statement.execute("UNLISTEN " + quotedIdentifier(channel));
    } catch (SQLException exception) {
      try {
        connection.abort(Runnable::run);
      } catch (Throwable abortException) {
        if (abortException != exception) {
  ...
  private boolean sleepBeforeTrigger(Set<String> queueNames) {
    ...
    try {
      ...
      if (!running) {
        ...
        incrementCounter("asyncJobQueue.wakeup.listener.triggerSleepInterrupted");
        logger.warn(
  ...
  private void closeConnection(Connection connection) {
    try {
      ...
      logger.warn(
          ...
          channel,
          exception);
      incrementCounter("asyncJobQueue.wakeup.listener.close.failed");
    }
  }
  ...
  private void incrementListenCounter(String result) {
    incrementCounter("asyncJobQueue.wakeup.listen", "provider", "postgres", "result", result);
  }
  ...
  private void incrementCounter(String name, String... tags) {
    try {
      ...
      try {
        logger.warn("Failed to record PostgreSQL async queue wakeup metric {}", name, failure);
      } catch (Throwable loggingFailure) {
        if (isJvmFatal(loggingFailure)) {
          throw (Error) loggingFailure;
        }
        // Diagnostics must not reintroduce a meter failure into listener control flow.
      }
    }
  }
