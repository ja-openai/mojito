...
final class AsyncJobQueueValidation {
  ...
  // Queue times are UTC from the epoch onward; database calendars disagree on ancient dates.
  static final Instant DATABASE_TIMESTAMP_MIN = Instant.EPOCH;
  ...
  static AsyncJobQueueProperties validateProperties(
    ...
    for (Map.Entry<String, AsyncJobQueueProperties.QueueSettings> queueEntry :
      ...
      if (queueEntry.getValue() != null) {
        ...
        if (!queueEntry.getValue().isConsumerEnabled() && STORE_IN_MEMORY.equals(store)) {
          throw new IllegalArgumentException(
              "consumerEnabled=false requires store=jdbc; remote consumers cannot drain in-memory queue "
                  + queueEntry.getKey());
        }
      }
    }
  ...
  static String validateJobData(String jobData) {
    validateStoredJobData(jobData);
    for (int i = 0; i < jobData.length(); i++) {
      char current = jobData.charAt(i);
      if (current == '\0') {
        throw new IllegalArgumentException("jobData must not contain NUL");
      }
      if (Character.isHighSurrogate(current)) {
        if (i + 1 >= jobData.length() || !Character.isLowSurrogate(jobData.charAt(i + 1))) {
          throw new IllegalArgumentException("jobData must contain only well-formed UTF-16");
        }
        i++;
  ...
  // Retain the original read contract so old payloads do not poison claim batches or inspection.
  static String validateStoredJobData(String jobData) {
    Objects.requireNonNull(jobData);
    if (jobData.length() > JOB_DATA_MAX_LENGTH) {
      throw new IllegalArgumentException(
          "jobData must be at most " + JOB_DATA_MAX_LENGTH + " UTF-16 code units");
    }
    return jobData;
  }
  ...
  static String truncateLastError(String lastError) {
    ...
    StringBuilder summary = new StringBuilder(Math.min(lastError.length(), LAST_ERROR_MAX_LENGTH));
    for (int i = 0; i < lastError.length() && summary.length() < LAST_ERROR_MAX_LENGTH; i++) {
      char current = lastError.charAt(i);
      boolean paired =
          Character.isHighSurrogate(current)
              && i + 1 < lastError.length()
              && Character.isLowSurrogate(lastError.charAt(i + 1));
      // PostgreSQL text rejects NUL; malformed UTF-16 also has no portable JDBC representation.
      if (current == 0 || (Character.isSurrogate(current) && !paired)) {
        if (summary.length() + 6 > LAST_ERROR_MAX_LENGTH) {
          break;
        }
        summary.append("\\u").append(HexFormat.of().toHexDigits(current));
      } else if (paired) {
        if (summary.length() + 2 > LAST_ERROR_MAX_LENGTH) {
          break;
        }
        summary.append(current).append(lastError.charAt(++i));
      } else {
  ...
  static Duration validateRetentionAge(Duration retentionAge) {
    Objects.requireNonNull(retentionAge, "retentionAge");
    if (retentionAge.isNegative()
        || retentionAge.isZero()
        || retentionAge.compareTo(Duration.ofMillis(RETENTION_AGE_MS_MAX)) > 0) {
      throw new IllegalArgumentException(
          "retentionAge must be positive and <= " + RETENTION_AGE_MS_MAX + " milliseconds");
    }
    return retentionAge;
  }
