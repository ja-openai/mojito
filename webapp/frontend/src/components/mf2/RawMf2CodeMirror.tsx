...
export type RawMf2CodeMirrorHandle = {
  blur: () => void;
  focus: () => void;
};
...
export const RawMf2CodeMirror = forwardRef<RawMf2CodeMirrorHandle, Props>(function RawMf2CodeMirror(
  {
    active,
  ...
  useEffect(() => {
    ...
    // CodeMirror is created once when first shown; later effects synchronize changing props.
  ...
  return linter(
    (view) => {
      ...
      return (parsed.diagnostics ?? []).map((diagnostic) =>
        codeMirrorDiagnostic(diagnostic, source.length),
      );
    },
    { delay: 250 },
  );
...
function codeMirrorDiagnostic(diagnostic: Record<string, unknown>, docLength: number): Diagnostic {
  ...
  const code = diagnosticText(diagnostic.code) ?? 'mf2';
  const message = diagnosticText(diagnostic.message) ?? code;
  return {
...
function codeMirrorDiagnosticRange(diagnostic: Record<string, unknown>, docLength: number) {
  ...
  const rawEnd =
    diagnostic.end == null || diagnostic.end === diagnostic.start ? fallbackEnd : diagnostic.end;
...
function applyRawTextTool(view: EditorView, tool: RawTextTool) {
  ...
  const cursor =
    tool.wrap && range.empty ? range.from + tool.wrap[0].length : range.from + insert.length;
  view.dispatch({
    ...
    autocompletion({
      ...
      override: [
        (context: CompletionContext) =>
          rawPlaceholderCompletionSource(context, namesProvider, sourcesProvider),
      ],
    }),
    EditorView.updateListener.of((update) => {
