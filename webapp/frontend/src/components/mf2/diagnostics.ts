...
const ROW_HELPER_DIAGNOSTIC_CODES = new Set(['missing-locale-plural-variant']);
...
export function diagnosticsNearActiveEditor(
  ...
  const activeDiagnostics = diagnostics.filter((diagnostic) =>
    diagnosticMatchesForm(diagnostic, activeFormLabel),
  );
