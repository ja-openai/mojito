import './mf2-translation-editor.css';
...
import { Mf2ProseMirrorEditor, type Mf2ProseMirrorEditorHandle } from './Mf2ProseMirrorEditor';
import {
  addLocalePluralRows,
  cloneModel,
  diagnosticsFor,
  ...
  type SourceDeclaredLocalePluralSelector,
...
export type Mf2TranslationEditorHandle = TranslationEditorHandle;
...
>(function Mf2TranslationEditor(
  ...
  const rawEditorRef = useRef<RawMf2CodeMirrorHandle | null>(null);
  ...
  useImperativeHandle(
    ...
    () => ({
      blur() {
        if (mode === 'raw') rawEditorRef.current?.blur();
        else proseMirrorRef.current?.blur();
      },
      focus() {
  ...
  const sourceParsed = useMemo(
    () => parseMf2(source, argValues, locale, { includeRuntimeDiagnostics: false }),
    [argValues, locale, source],
  );
  const targetParsed = useMemo(
    () =>
      parseMf2(target, argValues, locale, {
        includeRuntimeDiagnostics: showPreview,
      }),
    [argValues, locale, showPreview, target],
  );
  ...
  const lastValidTargetModel =
    lastValidTargetModelRef.current?.source === source
  ...
  const diagnostics = useMemo(
    ...
    [locale, sourceModel, sourceParsed.diagnostics, targetModel, targetParsed.diagnostics],
  );
  ...
  const sourcePlaceholderSources = useMemo(
    () => placeholderSourceByName(sourceModel),
    [sourceModel],
  );
  ...
  const sourceLocaleSelectorCandidates = useMemo(
    () => (targetModel?.type === 'message' ? sourceDeclaredLocalePluralSelectors(sourceModel) : []),
    [sourceModel, targetModel?.type],
  );
  const invalidLocaleRows = useMemo(
    () => invalidLocalePluralRows(targetModel, locale),
    [locale, targetModel],
  );
  const activeSourceVariant = sourceVariantForTargetModel(
    sourceModel,
    editableModel,
    activeTargetKeys,
  );
  ...
  const activeSourcePlaceholderSources = useMemo(
    () => placeholderSourcesInPattern(activeSourcePattern),
    [activeSourcePattern],
  );
  ...
  const activeSourceFormLabel = sourceFormLabelForTargetModel(
    sourceModel,
    editableModel,
    activeTargetKeys,
  );
  const activeEditorMinLines = Math.max(
    1,
  ...
  const effectiveLocaleOptions = useMemo(() => {
    return localeOptionsWithCurrent(
      localeOptions?.length ? localeOptions : DEFAULT_LOCALE_OPTIONS,
      locale,
    );
  ...
  const moveActiveForm = useCallback(
    (delta: number, focusEditor = true) => {
  ...
  const placeholderSourceForName = useCallback(
    (name: string) => {
  ...
  function startLocalePluralForms(selectorName: string) {
    if (readOnly) return;
    const result = promoteSimpleTargetToLocalePluralSelect(
      sourceModel,
      targetModel,
      selectorName,
      locale,
  ...
  function restoreRawMissingPlaceholders() {
    ...
    if (next.type === 'select') {
      const variant = next.variants[activeVariantIndex];
      if (!variant) return;
      variant.value = patternWithRestoredPlaceholders(
        variant.value,
        missing,
        placeholderSourceForName,
      );
    } else {
      next.pattern = patternWithRestoredPlaceholders(
        next.pattern,
        missing,
        placeholderSourceForName,
      );
            ...
            {richEditorCanMutate ? (
              <>
                <LocaleSelectorStarter
                  ...
                  locale={locale}
                  onStart={startLocalePluralForms}
                />
                <RowHelper
              ...
              {Object.keys(argValues)
                ...
                .map((name) => (
                  <label key={name}>
                    <span>{name}</span>
                    <input
                      value={argumentInputValue(argValues[name])}
                      onChange={(event) =>
      ...
      {showDebugTools ? (
        <details className="mf2-debug" open={debug}>
          ...
          <pre>
            {JSON.stringify(
              {
                model: targetModel?.rustModel ?? null,
                output: targetParsed.output,
                parts: targetParsed.parts,
              },
              null,
              2,
            )}
...
function argumentInputValue(value: unknown) {
...
function FormIssueBadge({ diagnostics }: { diagnostics: Array<EditorDiagnostic> }) {
  ...
  return (
    <span className={`mf2-form-issue-badge mf2-form-issue-badge-${errors ? 'error' : 'warning'}`}>
      {label}
    </span>
  );
}
...
function InlineDiagnostics({
  ...
  return (
    <div className="mf2-inline-diagnostics" aria-live="polite">
      ...
      {hiddenCount ? (
        <span className="mf2-inline-issue-more">+{hiddenCount} more below</span>
      ) : null}
    </div>
  );
}
...
function RowHelper({
  ...
  const removeInvalidRowsLabel = rowHelperRemoveLabel(
    removableInvalidFormCount,
    removableInvalidSummary,
  );
  ...
  return (
    <div className="mf2-row-helper">
      {invalidRows.length ? (
        <span>
          {removableInvalidRows.length
            ? 'Remove blank forms invalid for this locale'
            : 'Review forms invalid for this locale'}
          {invalidSummary ? (
            <>
              : <strong>{invalidSummary}</strong>
            </>
          ) : null}
        </span>
      ) : null}
      {removableInvalidFormCount ? (
        <button aria-label={removeInvalidRowsLabel} type="button" onClick={onRemoveInvalidRows}>
      ...
      {suggestions.length ? (
        <>
          ...
          <button aria-label={addLocaleRowsLabel} type="button" onClick={onAddLocaleRows}>
...
function LocaleSelectorStarter({
  candidates,
  locale,
  onStart,
}: {
  candidates: Array<SourceDeclaredLocalePluralSelector>;
  locale: string;
  onStart: (selectorName: string) => void;
}) {
  if (!candidates.length) return null;
  return (
    <div className="mf2-row-helper mf2-selector-starter">
      <span>
        This translation can add flat, locale-specific plural forms without changing the source.
      </span>
      {candidates.map((candidate) => (
        <button
          aria-label={`Add ${locale} plural forms for $${candidate.name}`}
          key={candidate.name}
          onClick={() => onStart(candidate.name)}
          type="button"
        >
          Add {locale} forms for ${candidate.name}
        </button>
      ))}
    </div>
  );
  ...
  if (keys.includes(' or ')) {
    return interleave(
      keys
        ...
        .map((key) => key.trim())
        .map((key) => (
          <span className="mf2-shortcut-keys" key={key}>
            {shortcutKeys(key)}
          </span>
        )),
  ...
  if (keys.includes('/')) {
    return interleave(
      keys
        .split('/')
        .map((key) => key.trim())
        .map((key) => <kbd key={key}>{shortcutKeyLabel(key)}</kbd>),
...
function interleave(items: Array<ReactNode>, separator: ReactNode) {
  return items.flatMap((item, index) =>
    index ? [<span key={`separator-${index}`}>{separator}</span>, item] : [item],
  );
}
