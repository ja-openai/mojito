...
A genuinely first target locale obtains native cardinal categories directly
from existing ICU4J in Java and Mojito's existing complete MF2 plural rules in
Rust. Neither converter copies or maintains a Unicode locale table. The independent
Node ICU oracle verifies representative language families, script and region
variants, deprecated aliases, and unsupported-language rejection without
allowing silent fallback. Existing same-language catalog
...
from existing plural rules, retain source-owned scalar `%2$n` and plural `%3$n`,
...
from the same native plural rules. The source's own `argNum`, `formatSpecifier`,
...
Java ICU and Rust MF2 rule versions are maintained by their existing
dependencies and generated-rule update process. Both can support locales
even when the installed Node test runtime is older. Representative native
Xcode/Foundation and Node fixtures catch changes to category sets, aliases,
regional behavior, source-template ownership, and unsupported-language safety.

Regional identity is more specific than the set of available categories.
Original Brazilian `pt_BR` and European `pt-PT` records each require native
...
and every UTF-8/UTF-16 source byte. Safe region/script fallback to an ICU
...
`DUPLICATE_LOCALE`, while preserving distinct Chinese scripts and rejecting
unsupported or undefined language roots.
...
source bytes. ICU resolves only native-proven deprecated language
...
and unsupported language roots are never guessed.
