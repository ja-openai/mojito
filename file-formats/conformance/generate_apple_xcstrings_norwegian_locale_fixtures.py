#!/usr/bin/env python3
"""Generate source-owned grandfathered Bokmål/Nynorsk Xcode locale fixtures."""
...
STEM = "catalog-first-norwegian-grandfathered-locales"
BOKMAL_ID = "harbor.first.norwegian.bokmal.grandfathered🧭"
NYNORSK_ID = "harbor.first.norwegian.nynorsk.grandfathered🧭"
PROTECTED_ID = "Private Norwegian language harbor"
LOCALES = {BOKMAL_ID: "no-bok", NYNORSK_ID: "no-nyn"}
LABELS = {BOKMAL_ID: "Bokmål", NYNORSK_ID: "Nynorsk"}
...
def document() -> dict[str, object]:
    strings = {
        identifier: {
            "comment": "Grandfathered Bokmål/Nynorsk identifiers remain source-owned",
            "localizations": {
...
def skeleton(source: str, encoding: str, locale: str) -> dict[str, object]:
    ...
    for identifier, entry in document()["strings"].items():
        ...
        if locale in entry["localizations"]:
            ...
            if match is None:
                raise RuntimeError(
                    f"Missing grandfathered Norwegian locale: {identifier}"
                )
    ...
    return [
        {
            ...
            "expected": (
                f"Rowan {f'{count:,}'.replace(',', chr(0xA0))}  "
                f"{LABELS[identifier]} {category}"
            ),
        }
        for identifier in LOCALES
...
def main() -> None:
    ...
    identifier = "apple-xcstrings-first-norwegian-bokmal-nynorsk-grandfathered-locales"
    ...
    manifest["cases"].append(
        {
            ...
            "okapi": {
                ...
                "reason": (
                    "Legacy routing cannot distinguish grandfathered Bokmål/Nynorsk "
                    "source spelling, modern compiler bundles, region-sensitive language "
                    "aliases, hidden arguments, or original source-template bytes."
                ),
            },
        }
    )
    prefix = "apple-xcstrings-source-skeleton-inserts-first-norwegian-grandfathered-"
    manifest["sourceSkeletons"] = [
        case
        for case in manifest["sourceSkeletons"]
        if not case["id"].startswith(prefix)
    ]
    for label, locale, requested in (
        ("bokmal", "no-bok", "nb"),
        ("nynorsk", "no-nyn", "nn"),
    ):
            ...
            manifest["sourceSkeletons"].append(
                {
                    ...
                    **({"encoding": encoding} if suffix else {}),
                    ...
                    "xcstringsGrandfatheredLocale": label,
                    "xcstringsRuntimeLocale": requested,
                    "xcstringsFormattingLocale": requested,
                    "input": f"fixtures/apple/{STEM}.xcstrings",
