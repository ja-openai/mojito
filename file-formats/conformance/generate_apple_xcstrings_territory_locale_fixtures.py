#!/usr/bin/env python3
"""Generate Xcode-owned obsolete territory and numeric-world region fixtures."""
...
STEM = "catalog-first-english-territory-locales"
BRITISH_ID = "harbor.first.english.british.obsolete🧭"
WORLD_ID = "harbor.first.english.world.numeric🧭"
PROTECTED_ID = "Private English territory harbor"
LOCALES = {BRITISH_ID: "en-UK", WORLD_ID: "en-001"}
LABELS = {BRITISH_ID: "British", WORLD_ID: "World"}
...
def document() -> dict[str, object]:
    strings = {
        identifier: {
            "comment": "Obsolete territories and numeric world regions remain source-owned",
            "localizations": {
...
def skeleton(source: str, encoding: str, locale: str) -> dict[str, object]:
    ...
    for identifier, entry in document()["strings"].items():
        ...
        if locale in entry["localizations"]:
            ...
            if match is None:
                raise RuntimeError(f"Missing territory locale null: {identifier}")
...
def main() -> None:
    ...
    identifier = (
        "apple-xcstrings-first-english-obsolete-territory-and-numeric-world-locales"
    )
    ...
    manifest["cases"].append(
        {
            ...
            "okapi": {
                ...
                "reason": (
                    "Legacy routing cannot preserve Xcode-owned obsolete British territory "
                    "spelling, its modern native bundle, numeric world-region identity, "
                    "hidden arguments, or original source-template bytes."
                ),
            },
        }
    )
    prefix = "apple-xcstrings-source-skeleton-inserts-first-english-territory-"
    ...
    for label, locale, requested, runtime in (
        ("british", "en-UK", "en-GB", "en-GB"),
        ("world", "en-001", "en-001", "en-001"),
    ):
            ...
            manifest["sourceSkeletons"].append(
                {
                    ...
                    **({"encoding": encoding} if suffix else {}),
                    ...
                    "xcstringsTerritoryLocale": label,
