#!/usr/bin/env python3
"""Generate native-verified Brazilian/European Portuguese first-locale plurals."""
...
STEM = "catalog-first-portuguese-regional-locales"
BRAZIL_ID = "harbor.first.brazilian.portuguese🧭"
PORTUGAL_ID = "harbor.first.european.portuguese🧭"
PROTECTED_ID = "Private regional Portuguese harbor"
LOCALES = {BRAZIL_ID: "pt_BR", PORTUGAL_ID: "pt-PT"}
NAMES = {BRAZIL_ID: "Brasil", PORTUGAL_ID: "Portugal"}
CATEGORIES = ("many", "one", "other")
TRANSLATIONS = {
    identifier: "{count, plural, "
    + " ".join(
        f"{category} {{{{arg1}} {{count}}  {NAMES[identifier]} {category}}}"
...
def document() -> dict[str, object]:
    strings = {
        identifier: {
            "comment": "Brazil and Portugal keep distinct zero-selection behavior",
            "localizations": {
...
def inserted(identifier: str) -> dict[str, object]:
    return plural(
        {
            category: f"%2$@ %1$lld %3$n {NAMES[identifier]} {category}"
            for category in CATEGORIES
        },
...
def skeleton(source: str, encoding: str, target_locale: str) -> dict[str, object]:
    ...
    for identifier, entry in document()["strings"].items():
        ...
        if target_locale in entry["localizations"]:
            ...
            if match is None:
                raise RuntimeError(f"Missing regional locale null: {identifier}")
...
def runtime_samples(locale: str) -> list[dict[str, object]]:
    result = []
    for identifier, country in NAMES.items():
        for count, category in (
            (0, "one" if locale == "pt_BR" else "other"),
            (1, "one"),
            (2, "other"),
            (1_000_000, "many"),
        ):
            formatted = f"{count:,}".replace(
                ",", "." if locale == "pt_BR" else "\u00a0"
            )
            result.append(
                {
                    "message": identifier,
                    "arguments": [count, "Rowan", 0],
                    "expected": f"Rowan {formatted}  {country} {category}",
                }
            )
    return result
...
def main() -> None:
    ...
    identifier = "apple-xcstrings-first-brazilian-european-regional-plural-locales"
    ...
    manifest["cases"].append(
        {
            ...
            "okapi": {
                ...
                "reason": (
                    "Legacy routing cannot distinguish Brazilian and European Portuguese "
                    "first-locale plural selection, preserve exact hyphen/underscore catalog "
                    "spelling, or maintain protected original bytes and hidden arguments."
                ),
            },
        }
    )
    prefix = "apple-xcstrings-source-skeleton-inserts-first-regional-portuguese-"
    manifest["sourceSkeletons"] = [
        case
        for case in manifest["sourceSkeletons"]
        if not case["id"].startswith(prefix)
    ]
    for label, locale in (("brazil", "pt_BR"), ("portugal", "pt-PT")):
            ...
            manifest["sourceSkeletons"].append(
                {
                    ...
                    **({"encoding": encoding} if suffix else {}),
                    ...
                    "xcstringsRegionalLocale": label,
                    "xcstringsRuntimeLocale": locale,
                    "xcstringsFormattingLocale": locale,
                    "input": f"fixtures/apple/{STEM}.xcstrings",
