...
const representativePluralLocales = new Map([
  ...
  ["sr-Latn", ["few", "one", "other"]],
  ["yi", ["one", "other"]],
  ["zh-Hans", ["other"]],
  ["zh-Hant", ["other"]],
]);
for (const [locale, expected] of representativePluralLocales) {
  if (Intl.PluralRules.supportedLocalesOf([locale]).length !== 1) {
    throw new Error(`${locale}: representative locale is unsupported by Node ICU`);
  }
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      `${locale}: expected plural categories ${JSON.stringify(expected)} ` +
        `differ from actual Node ICU categories ${JSON.stringify(actual)}`,
    );
  }
}
for (const locale of ["und", "zz"]) {
  if (Intl.PluralRules.supportedLocalesOf([locale]).length !== 0) {
    throw new Error(`${locale}: unsupported locale must not silently use fallback rules`);
  }
}
...
console.log(
  `FormatJS verified ${representativePluralLocales.size} representative ICU locales, ` +
