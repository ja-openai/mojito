...
def main() -> None:
    ...
    assert set(cardinal) == {
        ...
        "source",
        "cardinalCategories",
    }, "Unversioned portable CLDR cardinal-category provenance"
    assert cardinal["schemaVersion"] == 1 and cardinal["cldrVersion"] == "46"
    assert cardinal["unicodeVersion"] == "16.0.0"
    assert cardinal["source"] == (
        "https://github.com/unicode-org/cldr-json/blob/46.0.0/"
        "cldr-json/cldr-core/supplemental/plurals.json"
    ), "CLDR rule provenance must name its exact immutable release tag"
    assert hashlib.sha256(
        (ROOT / "cldr-cardinal-categories.v1.json").read_bytes()
    ).hexdigest() == (
        "87b6497db3b123ef104f365499a4a3f8967384679f246794fc460631d0d3175a"
    ), "Pinned CLDR categories changed without a reviewed release upgrade"
    assert len(cardinal["cardinalCategories"]) == 219
    assert cardinal["cardinalCategories"]["und"] == ["other"]
    for locale, categories in cardinal["cardinalCategories"].items():
        assert re.fullmatch(r"[a-z]{2,3}(?:-[A-Z]{2})?", locale), (
            f"Unsupported pinned CLDR locale {locale}"
        )
        assert categories == sorted(categories) and "other" in categories
        assert set(categories) <= {"zero", "one", "two", "few", "many", "other"}
    java_categories = (
        ROOT.parent.parent
        / "common/src/main/resources/com/box/l10n/mojito/fileformat/cldr-cardinal-categories.v1.json"
    )
    assert (
        java_categories.read_bytes()
        == (ROOT / "cldr-cardinal-categories.v1.json").read_bytes()
    ), "Java and Rust must consume the identical version-pinned CLDR category source"
    seen: set[str] = set()
    ...
    for case in skeletons:
        ...
        if target_locale := skeleton.get("appleTargetLocale"):
                    ...
                    if "substitutions" in localized_target:
                        ...
                        for selector, definition in localized_substitutions.items():
                            ...
                            if not evidence and case.get(
                                ...
                                evidence = set(
                                    cardinal["cardinalCategories"].get(
                                        target_locale.replace("_", "-"),
                                        cardinal["cardinalCategories"].get(
                                            target_locale.replace("_", "-").split("-")[
                                                0
                                            ],
                                            [],
                                        ),
                        ...
                        for device, branch in localized_devices.items():
                            ...
                            if case.get("xcstringsFirstLocaleDevices"):
                                ...
                                assert set(plural) == required, (
                                    ...
                                    "device plural differs from pinned CLDR categories"
                                )
                            assert all(
            ...
            if case.get("xcstringsTargetPluralInsertion"):
                ...
                if not evidence and case.get("xcstringsFirstLocaleCategories"):
                    cardinal_locale = target_locale.replace("_", "-")
                    language, *suffix = cardinal_locale.split("-")
                    if case.get("xcstringsDeprecatedLocale"):
                        language = {"iw": "he", "in": "id", "ji": "yi"}.get(
                            language, language
                        )
                        cardinal_locale = "-".join([language, *suffix])
                    evidence = set(
                        cardinal["cardinalCategories"].get(
                            cardinal_locale,
                            cardinal["cardinalCategories"].get(language, []),
                        )
                    )
                    assert all(
        ...
        if case.get("xcstringsFirstLocaleCategories"):
            locale = case["xcstringsTargetLocale"].replace("_", "-")
            assert locale in cardinal["cardinalCategories"] or (
                locale.split("-")[0] in cardinal["cardinalCategories"]
            ), f"{case_id}: first-locale insertion lacks pinned CLDR provenance"
