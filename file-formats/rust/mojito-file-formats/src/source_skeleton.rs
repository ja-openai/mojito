...
const CLDR_CARDINAL_CATEGORIES: &str =
    include_str!("../../../conformance/cldr-cardinal-categories.v1.json");
...
pub(crate) fn extract(
    ...
    match format {
        ...
        FileFormat::Html => crate::html::extract(bytes, false, true),
        _ => Err(error(
...
pub(crate) fn render(
    ...
    match skeleton.source_format {
        ...
        "html" => crate::html::render(skeleton, translations),
        _ => Err(error(
            "UNSUPPORTED_SKELETON_FORMAT",
...
pub(crate) fn cardinal_categories(locale: &str) -> HashSet<String> {
    ...
    let manifest = MANIFEST.get_or_init(|| {
        serde_json::from_str(CLDR_CARDINAL_CATEGORIES).expect("pinned CLDR cardinal manifest")
    });
    if manifest["schemaVersion"] != 1
        || manifest["cldrVersion"] != "46"
        || manifest["unicodeVersion"] != "16.0.0"
        || manifest["source"]
    ...
    let normalized_locale = locale
        ...
        .map(|(index, component)| {
            ...
            } else if component.len() == 2 || component.len() == 3 {
                component.to_ascii_uppercase()
            } else if component.len() == 4 {
    ...
    let language = normalized_locale
        .split('-')
        .next()
        .expect("normalized Xcode target locale");
    if language == "und" {
        return HashSet::new();
    }
    let canonical_language = match language {
        "iw" => "he",
        "in" => "id",
        "ji" => "yi",
        _ => language,
    };
    manifest["cardinalCategories"]
        .get(&normalized_locale)
        .or_else(|| manifest["cardinalCategories"].get(canonical_language))
        .and_then(serde_json::Value::as_array)
        .map(|categories| {
            categories
                .iter()
