...
### Pulling Strings Missing From Local Source

JSON source catalogs can include translated text units that are active in Mojito but do not yet
exist in the local source file. This is useful for strings staged by the String Authoring workflow
before the corresponding code change lands.

To include missing entries from specific branches:

```bash
mojito pull -r MyRepo -ft FORMATJS_JSON_NOBASENAME \
  --pull-with-no-source-branches authoring/checkout
```

To select Mojito's unnamed default branch (the branch populated by pushes without `-b`), use the
source-less-specific option:
...
The null-branch selector can be combined with named branches:
...
Both branch selectors imply `--pull-with-no-source`; when combined, Mojito includes missing units
from either the default branch or one of the named branches. The literal value `null` passed to
`--pull-with-no-source-branches` remains a normal branch named `null`, not the unnamed default
branch. Passing `--pull-with-no-source` without either selector includes missing entries from all
active branches for each matched asset. Existing local entries always win.

This first version supports literal top-level ids in flat JSON and the FormatJS `defaultMessage`
shape. Nested/path-like ids, plurals, and JSON filter options that transform source content or ids
are rejected because Mojito cannot reconstruct their original file skeleton safely. The normal
`--status` and `--inheritance-mode` options still apply. The command still needs a matching local
source asset file; the option adds entries missing from that file rather than discovering
remote-only assets.
